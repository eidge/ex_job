defmodule ExJob.Queue.SimpleQueueTest do
  use ExUnit.Case
  ...
  describe "dequeue/1" do
    ...
    test "moves jobs to the working queue" do
      queue = SimpleQueue.from_list([job(1), job(2)])
  ...
  describe "done/3" do
    ...
    test "increments correct count" do
      job1 = job()
      job2 = job()
      queue = SimpleQueue.from_list([job1, job2])
  ...
  describe "from_list/1" do
    test "returns a new %SimpleQueue{} from the list" do
      queue = SimpleQueue.from_list([job(1), job(2)])
      assert {:ok, queue, %Job{arguments: [1]}} = Queue.dequeue(queue)
      assert {:ok, _queue, %Job{arguments: [2]}} = Queue.dequeue(queue)
    end

    test "returns an empty %SimpleQueue{} from an empty list" do
      queue = SimpleQueue.from_list([])
      assert Queue.size(queue) == 0
    end
  end
